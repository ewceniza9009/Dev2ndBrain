...
const SnippetDetail: React.FC<SnippetDetailProps> = ({ snippet }) => {
  const { updateSnippet, deleteSnippet, syncSnippetToGist } = useSnippetStore();
    ...
    <div className="p-6">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex space-x-2 flex-shrink-0">
            {snippet.gistId ? (
                <a href={`https://gist.github.com/${snippet.gistId}`} target="_blank" rel="noopener noreferrer" className="px-4 py-2 text-sm bg-green-600 text-white rounded-lg">View on Gist</a>
            ) : (
