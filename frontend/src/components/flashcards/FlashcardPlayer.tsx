...
    const [sessionStats, setSessionStats] = useState<Record<string, number>>({});
    ...
    // VITAL FIX: Fetch cards only ONCE when the session starts.
    useEffect(() => {
        const startSession = async () => {
            let cardsToReview: Flashcard[];
            const allCardsInDeck = await db.flashcards.where({ deckId }).filter(c => !c.isDeleted).toArray();

            if (reviewMode === 'all') {
                cardsToReview = allCardsInDeck;
            } else {
                cardsToReview = allCardsInDeck.filter((c: Flashcard) => new Date(c.nextReview) <= new Date());
            }
...
        // Update the card in the database in the background
...
        ]);
        ...
        // This now works correctly because the queue is managed locally in state.
...
    if (reviewQueue.length > 0 && queue.length === 0 && !currentCard) {
        return (
            <div className="text-center">
...
                        </div>
                    ) : (
                      <p className="text-gray-500">No cards to review in this mode.</p>
                    )}
                </div>
            </div>
...
                    {Math.max(0, reviewQueue.length - queue.length)} / {reviewQueue.length} cards reviewed
                </p>
            </div>
...
            <div className="mt-6">
                {!isFlipped ? (
                    <button disabled={!currentCard} onClick={() => setIsFlipped(true)} className="flex items-center justify-center space-x-2 px-6 py-3 bg-blue-600 rounded-lg text-white text-lg font-semibold hover:bg-blue-700 shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50">
